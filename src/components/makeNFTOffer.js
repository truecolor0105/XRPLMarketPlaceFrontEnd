...
import { BiArchiveOut, BiTransfer } from "react-icons/bi";
...
export default function Createnft() {
    ...
    const sellReducer = useSelector((state) => state.sellReducer);
        ...
        <>
            <div className="md:flex mt-24 py-12 border-t-2 gap-10 border-t-white text-center md:text-start">
                <div className="w-full md:w-4/6 ">
                    <img src={metadata.nftImgPath}  />
                </div>
                {sellReducer ? <NFTsell nftID={id}/> :

                    <div className="w-full   text-start">
                        <div className="text-4xl text-lime-500 font-bold text-center"> MAKE AN OFFER </div>
                        ...
                        <div className="flex text-xl text-white gap-6 py-4">
                            ...
                            <div className="font-bold">{metadata.nftRoyalty}%</div>
                        </div>
                        ...
                        <button className="w-full flex text-center items-center my-4 justify-center rounded-xl py-4 bg-[#edb731] gap-4 transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 hover:bg-[#f3ec12] duration-300" onClick={() => dispatch(opensellnft())}>
                            <BiArchiveOut style={{ fontSize: "24px" }} />
                            <div className="text-xl font-bold">Sell this NFT</div>
                        </button>
                        <button className="w-full flex text-center items-center my-4 justify-center rounded-xl py-4 bg-[#edb731] gap-4 transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 hover:bg-[#f3ec12] duration-300">
                            <BiTransfer style={{ fontSize: "24px" }} />
                            <div className="text-xl font-bold" onClick={() => dispatch(closesellnft())}>Transfer NFT</div>
                        </button>
                    </div>
                }
            </div>
            <div className="block md:flex items-start text-start gap-3">
                ...
                <div className="w-full">
                    <div className="flex py-3 items-center w-full gap-1">
                        <FaUserAlt style={{ color: "white", borderRadius: "50%", border: "2px solid #edb731", fontSize: "32px" }} />
                        <div className="px-2 text-center text-white text-xl">{user.account}</div>
                    </div>
